...
public class ProductServiceAddTest {
    ...
    public void testAddProduct_Fail_TitleIsNull() throws IOException {
        ...
        // Bổ sung kiểm tra DB: không có sản phẩm nào với title null
        // (Vì không có id, nên kiểm tra tổng số lượng sản phẩm với title null)
        long count = (long) entityManager.createQuery("SELECT COUNT(p) FROM Product p WHERE p.title IS NULL").getSingleResult();
        assertEquals(0, count, "Không được có sản phẩm nào với title null trong DB");
