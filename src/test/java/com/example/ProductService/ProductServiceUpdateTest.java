...
public class ProductServiceUpdateTest {
    ...
    // Test: Update sản phẩm có productImage != null và size > 0 (phủ nhánh xóa ảnh cũ)
    @Test
    @Order(1001)
    @Transactional
    @Rollback
    public void testUpdateProduct_WithProductImage() throws IOException {
        Product product = new Product();
        product.setTitle("Sách Update Image");
        product.setPrice(BigDecimal.valueOf(100000));
        product.setShortDes("desc");
        product.setDetails("details");
        productService.add(product, new MockMultipartFile("avatar", "img.jpg", "image/jpeg", "img".getBytes()), new MockMultipartFile[]{});

        // Thêm ảnh vào product
        Product productInDb = productService.search(null).getData().get(0);
        // Giả lập có productImage
        productInDb.setProductImage(new java.util.HashSet<>());
        productService.update(productInDb, null, null);
        Assertions.assertNotNull(productInDb);
    }

    // Test: Update sản phẩm có productImage = null (phủ nhánh else)
    @Test
    @Order(1002)
    @Transactional
    @Rollback
    public void testUpdateProduct_WithNullProductImage() throws IOException {
        Product product = new Product();
        product.setTitle("Sách Update Null Image");
        product.setPrice(BigDecimal.valueOf(100000));
        product.setShortDes("desc");
        product.setDetails("details");
        productService.add(product, new MockMultipartFile("avatar", "img2.jpg", "image/jpeg", "img2".getBytes()), new MockMultipartFile[]{});

        // Giả lập productImage = null
        Product productInDb = productService.search(null).getData().get(0);
        productInDb.setProductImage(null);
        productService.update(productInDb, null, null);
        Assertions.assertNotNull(productInDb);
    }
}
