...
public class SaleorderServiceCreateTest {
    ...
    public void testCreateSaleorder_Fail_CodeIsNull() {
        ...
        // Bổ sung kiểm tra DB: không có saleorder nào với customer_email 'nguyenvana@example.com' và code null
        long count = (long) entityManager.createQuery("SELECT COUNT(s) FROM Saleorder s WHERE s.code IS NULL AND s.customer_email = :email")
            .setParameter("email", "nguyenvana@example.com").getSingleResult();
        assertEquals(0, count, "Không được có saleorder nào với code null trong DB");
    }
