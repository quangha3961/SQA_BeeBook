...
public class BlogServiceAddTest {
    ...
    public void testAddBlog_Fail_TitleIsNull() throws IOException {
        ...
        // Bổ sung kiểm tra DB: không có blog nào với title null
        long count = (long) entityManager.createQuery("SELECT COUNT(b) FROM Blog b WHERE b.title IS NULL").getSingleResult();
        assertEquals(0, count, "Không được có blog nào với title null trong DB");
    }
