...
public class UserServiceCreateTest {
    ...
    public void testCreateUser_Fail_UsernameIsNull() {
        ...
        // Bổ sung kiểm tra DB: không có user nào với email 'user4@example.com'
        long count = (long) entityManager.createQuery("SELECT COUNT(u) FROM User u WHERE u.email = :email")
            .setParameter("email", "user4@example.com").getSingleResult();
        assertEquals(0, count, "Không được có user nào với username null trong DB");
    }
}