...
public class AuthorServiceSearchTest {
    ...
    //Test Add 1: Truyền searchModel == null
    @Test
    @Order(7)
    @Transactional
    @Rollback
    public void testSearchWithNullSearchModel() {
        PagerData<Author> result = authorService.search(null);
    ...
    //Test Add 2: Truyền searchModel có cả id và keyword
    @Test
    @Order(8)
    @Transactional
    @Rollback
    public void testSearchWithIdAndKeyword() {
        SearchModel searchModel = new SearchModel();
        searchModel.id = 52; // id tồn tại
        searchModel.keyword = "Tố Hữu";
        PagerData<Author> result = authorService.search(searchModel);
        Assertions.assertFalse(result.getData().isEmpty(), "Phải có kết quả khi truyền cả id và keyword hợp lệ");
        Assertions.assertEquals(52, result.getData().get(0).getId(), "ID kết quả phải là 52");
    }
    //Test Add 3: Truyền searchModel.keyword rỗng
    @Test
    @Order(9)
    @Transactional
    @Rollback
    public void testSearchWithEmptyKeyword() {
        ...
        searchModel.keyword = "";
        PagerData<Author> result = authorService.search(searchModel);
        Assertions.assertNotNull(result, "Kết quả không được null nếu keyword rỗng");
