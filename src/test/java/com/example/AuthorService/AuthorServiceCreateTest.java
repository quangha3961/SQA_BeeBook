...
public class AuthorServiceCreateTest {
    ...
    public void testCreateAuthor_Fail_NameTooShort() {
        ...
        assertNull(savedAuthor, "Tác giả có tên ngắn không được lưu."); // Trường hợp nếu phương thức trả về null
        // Bổ sung kiểm tra DB: không có author nào với tên 'ABC'
        long count = (long) entityManager.createQuery("SELECT COUNT(a) FROM Author a WHERE a.name = :name")
            .setParameter("name", "ABC").getSingleResult();
        assertEquals(0, count, "Không được có tác giả nào với tên quá ngắn trong DB");
    }
